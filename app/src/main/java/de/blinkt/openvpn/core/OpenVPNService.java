...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public static String humanReadableByteCount(long bytes, boolean speed, Resources res) {
        ...
        int exp = Math.max(0, Math.min((int) (Math.log(bytes) / Math.log(unit)), 4));
        ...
        if (speed)
            switch (exp) {
                ...
                default:
                    return res.getString(R.string.tbits_per_second, bytesUnit);
            }
        else
            switch (exp) {
                case 0:
                ...
                default:
                    return res.getString(R.string.volume_tbyte, bytesUnit);
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && DISCONNECT_VPN.equals(intent.getAction())) {
            boolean doShow = Utils.getSettingSharePref("settings_general_show_closing_confirm_dialog", false, this);
            if(!doShow){
                ProfileManager.setConntectedVpnProfileDisconnected(this);
                if (this.getManagement() != null)
                    this.getManagement().stopVPN(false);
            }else{
                Intent disconnectVPN = new Intent(this, DisconnectVPN.class);
