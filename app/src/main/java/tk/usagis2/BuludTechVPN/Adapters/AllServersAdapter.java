 ...
 */
public class AllServersAdapter extends RecyclerSwipeAdapter<AllServersAdapter.SimpleViewHolder> {
    ...
    public class SimpleViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnCreateContextMenuListener, MenuItem.OnMenuItemClickListener{
        ...
        public SimpleViewHolder(View itemView, Context context) {
            ...
            btnSave = itemView.findViewById(R.id.bottom_button_save);

            btnConnect.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_CONNECT);
                }
            });
            ...
            btnSave.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_SAVE_TO);
                }
            });
        ...
        public void onCreateContextMenu(ContextMenu contextMenu, View view, ContextMenu.ContextMenuInfo contextMenuInfo) {
            ...
            MenuItem saveVPN = contextMenu.add(context.getResources().getString(R.string.context_save));
            ...
            saveVPN.setOnMenuItemClickListener(this);
        ...
        public boolean onMenuItemClick(MenuItem menuItem) {
            if(menuItem.getTitle().equals(context.getResources().getString(R.string.context_connect))){
                clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_CONNECT_THIS);
            }else if(menuItem.getTitle().equals(context.getResources().getString(R.string.context_connect_3rd))) {
                clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_CONNECT_3RD);
            }else if(menuItem.getTitle().equals(context.getResources().getString(R.string.context_save))) {
                clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_SAVE_TO);
            }else if(menuItem.getTitle().equals(context.getResources().getString(R.string.context_share))){
                clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_SHARE);
            }else if(menuItem.getTitle().equals(context.getResources().getString(R.string.context_sdcard))){
                clickCallback.onClickToButton(SimpleViewHolder.this.getAdapterPosition(), AllServersFragment.KEY_CODE_SAVE_TO_SD);
            }
            return true;
        }
    }
    ...
    public boolean filterData(){
        ...
        //filter
        String filterByCountry = Utils.getPref(DashBoard.PREF_FILTER_COUNTRY, "", context);
        String filterByHostName = Utils.getPref(DashBoard.PREF_FILTER_HOSTNAME, "", context);
        String filterByIP = Utils.getPref(DashBoard.PREF_FILTER_IP, "", context);
        ...
        //sort
        int sortBy = Utils.getPref(DashBoard.PREF_SORT_BY, 0, context);
        int sortOrderInt = Utils.getPref(DashBoard.PREF_SORT_ORDER, 0, context);
        boolean sortOrder;
        switch (sortOrderInt){
            case 1:
                sortOrder = true;
    ...
    public AllServersAdapter(Context context, List<VPNClass> objects, IVPNFragmentCallbacksForAdapter callback) {
        this.context = context;
    ...
    public SimpleViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_list_item_allserver, parent, false);
    ...
    public void onBindViewHolder(SimpleViewHolder viewHolder, int position) {
        ...
        if(item.CountryShort != null){
            Glide.with(context).load(Utils.getFlagDrawable(context, item.CountryShort.toUpperCase())).into(viewHolder.imageFlag);
        }else{
            Glide.with(context).load(R.mipmap.unknown).into(viewHolder.imageFlag);
        }
